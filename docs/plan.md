...
## Status: Active, wave 1 fully merged (2026-08-29). E1 shipped except the live-verify task (T1.6, deferred 3x -- DGX load too high on first two attempts, a cpuset-core scheduling dimension not yet understood on the third; not part of this wave's 10 dispatch units). E2-E5 all merged (#73 PR #103, #77 PR #90, #88 PR #94, #74 PR #93, #71 PR #101, #75 PR #96, #78 PR #97, #80 manifest PR #100, #80 signedness PR #95, #79 PR #92) and released as v1.19.0, deployed live on the DGX (`/healthz` -> 1.19.0). **None of the 9 fixes have been live-verified yet** -- that pass was queued and had not started when this session handed off; see docs/handover.md. All issues reopened (auto-closed by their PR's Fixes-line, against this plan's close-only-after-live-verify procedure) except #71 (no live repro exists for its podman-internal timing race) and #78 (PR left it open correctly). E6-E7 are outline epics pending their triggers, now unblocked.
...
- [x] T2.9 (issue #88) `DELETE /api/v1/pods/{name}` on a GPU-attached pod hung for ~7 minutes during T2.1b's live verification -- `podman pod stop` was retried three times, a `podman` child process parented by spark's own PID became a zombie (exited but never reaped), and an unrelated, unfiltered `sudo podman pod ps` also hung host-wide for the same window. Self-resolved without a `spark.service` restart; not yet root-caused. Investigate `StopPod`/`RemovePod`/`StreamPodLogs` in `internal/executor/podman.go` for a wedged-context or lock-contention path with no timeout of its own; a non-GPU pod deleted on the same fast timing would help isolate whether CDI/GPU teardown is the trigger. Owner: TBD  Est: 60m  verifies: [UC-105]  acc: [a targeted test or repro demonstrates the specific lock/timeout gap, and a fix bounds the stop/delete path's own wait so it cannot block host-wide podman pod-enumeration commands]  lane: agent  (completed 2026-08-29 -- PR #94 merged a54c05c, runPodmanBounded 20s timeout+5s WaitDelay; NOT yet live-verified, HIGH RISK per R7, queued)
- [x] T2.2 (issue #73) Reproduce with the exact manifest from the issue (a ~900-char `command[2]` with nested double quotes); trace where the argument is lost between `state.db`'s stored `spec_json` (already confirmed correct) and the actual podman invocation -- likely the same JSON-array `--entrypoint` encoding path #2.1 touches. Write a failing test first. Owner: TBD  Est: 45m  verifies: [UC-103]  acc: [the exact repro command from issue #73 reaches the container as a single intact argv element, verified via a test that execs a real echo/printf and checks output]  (completed 2026-08-29 -- root cause: buildRunArgs JSON-folded multi-token Command into --entrypoint. PR #103 merged 9821995; a first kazi attempt falsely claimed "already fixed", coordinator live-verified that claim false on the DGX before redispatch. NOT yet live-verified as fixed -- needs a release+deploy first, was still queued when this session handed off)
- [x] T2.3 (issue #73) Fix and add regression test using the exact reported script content (nested quotes, ~900 chars). Owner: TBD  Est: 30m  verifies: [UC-103]  acc: [go test ./internal/executor/... -race passes a new test built from the #73 repro manifest]  deps: [T2.2]  (completed 2026-08-29 -- TestBuildRunArgs_Issue73Repro added, PR #103)
- [x] T2.4 (issue #77) Investigate: reproduce the "pod completes instantly, zero container startup" sequence with the exact manifest from the issue (2-element `command`+`args`, `restartPolicy: Always`); determine whether it shares root cause with #73/#85 (same buildRunArgs path) or is a distinct intermittent failure (the issue notes the identical manifest shape worked once earlier in the same session, suggesting non-determinism, not a hard parse bug). Owner: TBD  Est: 60m  verifies: [UC-103]  acc: [either a reliable repro is found and asserted in a test, or the investigation produces a written root-cause hypothesis with specific next steps in docs/devlog.md]  lane: agent  (completed 2026-08-29 -- root cause: parseYAMLLines never handled a same-indent "- " block sequence, silently zeroed spec.Containers. PR #90 merged b4bc9de. NOT yet live-verified, queued)
- [x] T2.5 (issue #77) Fix if root-caused in T2.4; otherwise write up findings and open a narrower follow-up issue with what was ruled out. Owner: TBD  Est: 45m  verifies: [UC-103]  deps: [T2.4]  lane: agent  (completed 2026-08-29 -- fixed per T2.4's root cause, same PR #90)
- [x] T2.6 Also add regression coverage for the flag-skip-list's other gaps that #2.1's inline rewrite should have fixed as a side effect: `--cpuset-cpus`, `--user`, `--cap-add`, `--cap-drop` combined with a GPU device. Owner: TBD  Est: 20m  verifies: [UC-104]  acc: [go test ./internal/executor/... -race covers all four flags combined with a GPU-assigned pod and passes]  (completed 2026-08-29 -- independently verified: pre-existing TestBuildRunArgs_GPUDevicesWithOtherFlags, landed with #85's PR #86, already covers this. No new code needed)
- [x] T2.7 `go vet ./... && staticcheck ./... && go test ./... -race -timeout 120s -count=1` across all of E2's changes combined. Owner: TBD  Est: 15m  verifies: [infrastructure]  acc: [exit code 0 on all three commands]  deps: [T2.3, T2.5, T2.6, T2.9]  (completed 2026-08-29 -- go build/vet/staticcheck/test -race all clean on origin/main with all of E2 merged)
...
- [x] T3.1 Read `internal/api/pods_mutate.go`'s `handleApplyPod` and `internal/manifest/parse.go` to find where JSON bodies are (mis)handled -- confirm or refute the hypothesis that the hand-rolled YAML parser is silently accepting JSON as a degenerate/empty YAML document rather than erroring. Owner: TBD  Est: 30m  verifies: [UC-102]  acc: [a written root-cause note names the exact function and line where a JSON body produces a zero-value ParseResult without an error]  (completed 2026-08-29 -- confirmed: handleApplyPod never checked Content-Type, YAML parser's nested-block guard silently returned an empty root map)
- [x] T3.2 Fix: either make the parser JSON-aware (JSON is a YAML subset; the fix may be as small as detecting `{` as the first non-whitespace byte and routing to `encoding/json`), or make `handleApplyPod` reject unparseable/empty-result manifests with 400 instead of 201. Prefer making JSON actually work, since it is documented as supported. Owner: TBD  Est: 45m  verifies: [UC-102]  acc: [POST /api/v1/pods with Content-Type: application/json and a valid pod body returns 201 with a non-null pods array, and GET /api/v1/pods/{name} finds it]  deps: [T3.1]  (completed 2026-08-29 -- Parse sniffs first non-whitespace byte, routes {/[ through encoding/json. PR #93 merged d0504a7a. NOT yet live-verified, queued)
- [x] T3.3 Regression test: the exact JSON manifest shape from the issue, asserting a real pod is created and independently a malformed JSON body gets 400, not 201. Owner: TBD  Est: 30m  verifies: [UC-102]  acc: [go test ./internal/api/... -race covers both the valid-JSON-creates-pod and invalid-JSON-rejects-with-400 cases]  deps: [T3.2]  (completed 2026-08-29 -- TestApplyPod_JSON + TestApplyPod_MalformedJSON added, PR #93)
- [x] T3.4 `go vet ./... && staticcheck ./... && go test ./... -race -timeout 120s -count=1`. Owner: TBD  Est: 15m  verifies: [infrastructure]  acc: [exit code 0 on all three]  deps: [T3.3]  (completed 2026-08-29 -- go build/vet/staticcheck/test -race all clean on origin/main)
...
- [x] T4.1 (issue #71) In the DELETE path (`internal/api/pods_mutate.go`, and the equivalent NATS handler), classify "cgroup slice not loaded" / already-gone-cgroup errors from `podman pod rm` the same way `isNoSuchPod` is already classified elsewhere -- desired end state (pod gone) is already true, proceed with store/scheduler cleanup instead of returning 500 and aborting. Add a bounded retry on `rm` to also cover the race window itself. Owner: TBD  Est: 45m  verifies: [UC-105]  acc: [a DELETE that hits a simulated 'cgroup slice not loaded' error still returns deleted:true and the store record and scheduler reservation are gone]  (completed 2026-08-29 -- isCgroupCleanupRace/isPodAlreadyGone classify the podman cgroup-race error same as isNoSuchPod, bounded 3-attempt retry. PR #101 merged e17199e/3aa3822. NO live repro possible -- podman-internal timing race, not client-controllable; coverage is 4 fake-executor tests only, coordinator to decide close-on-tests-alone vs attempt trigger)
- [x] T4.2 (issue #71) Regression test using a fake executor that returns the exact "cgroup: Unit machine-libpod_pod_<id>.slice not loaded" error text on first `RemovePod` call. Owner: TBD  Est: 20m  verifies: [UC-105]  acc: [go test ./internal/api/... -race (and the equivalent bus test) cover this exact error string]  deps: [T4.1]  (completed 2026-08-29 -- TestDeletePodCgroupRaceTreatedAsSuccess/RetrySucceeds (api) + TestDeleteHandler_* (bus) added, PR #101)
- [x] T4.3 (issue #75) In `internal/reconciler/reconciler.go`'s `reconcilePending` (and its `Preempting` twin), treat `BackoffLimit == 0` as "fail on first CreatePod error" instead of "no limit enforced" -- drop the `> 0` guard or special-case `== 0` to call `terminateAfterStartFailure` immediately. Owner: TBD  Est: 30m  verifies: [UC-106]  acc: [a Job with backoffLimit: 0 whose CreatePod always fails reaches status=failed after exactly one attempt, not an unbounded retry loop]  (completed 2026-08-29 -- dropped BackoffLimit > 0 && guard at both CreatePod-failure call sites in reconcilePending. PR #96 merged de7b90e. NOT yet live-verified, queued)
- [x] T4.4 (issue #75) Regression test: fake executor whose CreatePod always errors, backoffLimit: 0, assert status transitions to Failed after attempt 1. Owner: TBD  Est: 20m  verifies: [UC-106]  acc: [go test ./internal/reconciler/... -race covers this exact case]  deps: [T4.3]  (completed 2026-08-29 -- TestCreatePodFailureTerminalAtBackoffLimitZero added, PR #96)
- [x] T4.5 (issue #78) Distinguish "pending, awaiting resources" from "podman has lost the pod" in the logs endpoint -- do not treat "no such pod" from `podman pod logs` as fatal while the pod's own status is still Pending; add a `pendingTimeout` config so callers can tell "still queued" from "gave up"; surface the shortfall message in the error returned to HTTP callers instead of a generic "lost" message. Owner: TBD  Est: 45m  verifies: [UC-107]  acc: [GET /api/v1/pods/{name}/logs on a pod that is status=pending due to resource shortfall returns an empty/not-started response, never an error implying the pod vanished]  (completed 2026-08-29 -- pendingLogTimeout (default 10m, --pending-log-timeout): within it /logs returns 200 empty, past it 503 naming the real shortfall. PR #97 merged 13042f7. Issue correctly left open by the PR itself. NOT yet live-verified, queued; 503/timeout path is unit-tested only)
- [x] T4.6 (issue #78) Regression test: a pod pending on resource shortfall, assert /logs does not error and /events surfaces the shortfall reason. Owner: TBD  Est: 20m  verifies: [UC-107]  acc: [go test ./internal/api/... -race covers this]  deps: [T4.5]  (completed 2026-08-29 -- 5 new tests added, PR #97)
- [x] T4.7 (issue #80, quick win 1) Add `GET /api/v1/pods/{name}/manifest` returning the exact submitted manifest bytes (or an equivalent structured form), sourced from the existing `spec_json`/source-path tracking already in `state.PodRecord`. Owner: TBD  Est: 30m  verifies: [UC-110]  acc: [GET /api/v1/pods/{name}/manifest returns the manifest that was POSTed for that pod, byte-equivalent modulo whitespace]  (completed 2026-08-29 -- PodRecord.RawManifest []byte threaded through all 3 ingestion paths (HTTP/NATS/watcher), new manifest_raw SQLite column, GET /api/v1/pods/{name}/manifest writes it verbatim. PR #100 merged f2859af. Two kazi grind attempts silently substituted a JSON-reconstruction for literal bytes even past a negative-space predicate meant to catch it -- shipped implementation hand-authored after the second rejection. NOT yet live-verified, queued)
- [x] T4.8 (issue #80, quick win 1) API test for the new endpoint (found pod, not-found pod). Owner: TBD  Est: 15m  verifies: [UC-110]  acc: [go test ./internal/api/... -race covers both cases]  deps: [T4.7]  (completed 2026-08-29 -- TestGetPodManifest/JSON/NotFound added, PR #100)
- [x] T4.9 (issue #80, quick win 2) Fix the `gpu 0 > -1 free` signedness bug in the shortfall-describing code (`internal/scheduler`'s `describeShortfall` or equivalent free-GPU computation) -- a negative "free" value should never be possible or displayed. Owner: TBD  Est: 20m  verifies: [UC-108, UC-109]  acc: [a shortfall message never displays a negative free-resource value; a targeted test asserts the free-GPU computation floors at 0]  (completed 2026-08-29 -- availableLocked()'s nonNegative() helper floors MemoryMB/GPUCount/GPUMemoryMB at 0 (deliberately not CPUMillis). PR #95 merged f30c7aa5, before this session's context compaction. NOT yet live-verified, queued)
- [x] T4.10 `go vet ./... && staticcheck ./... && go test ./... -race -timeout 120s -count=1` across all of E4's changes. Owner: TBD  Est: 15m  verifies: [infrastructure]  acc: [exit code 0 on all three]  deps: [T4.2, T4.4, T4.6, T4.8, T4.9]  (completed 2026-08-29 -- go build/vet/staticcheck/test -race all clean on origin/main with all of E4 merged)
...
- [x] T5.1 Reproduce: a high-priority pod behind more than 3 lower-priority, non-thrashed pods; confirm it never schedules due to the anti-thrash cap, and confirm the misleading event message. Write the failing test first. Owner: TBD  Est: 30m  verifies: [UC-108]  acc: [a test with 4+ eligible lower-priority victims and a high-priority pending pod shows Pending forever under current code]  (completed 2026-08-29 -- confirmed the starvation via isAntiThrashed/recordPreemption keyed victim-only)
- [x] T5.2 Fix: this needs a real design call, not a mechanical patch -- raising or removing the per-victim anti-thrash cap trades starvation-prevention for the original thrash-prevention the cap exists for. Do not silently pick a resolution; if the fix isn't obvious from the issue alone (e.g. cap per-pod-being-scheduled rather than global, or extend the anti-thrash window instead of the count), stop and report the trade-off rather than guessing. Also fix the event message to name the real shortfall instead of "evicting N candidates" when the cap is what's blocking, not availability. Owner: TBD  Est: 60m  verifies: [UC-108]  acc: [the T5.1 test goes green without introducing a preemption-storm regression test failure]  deps: [T5.1]  lane: agent  (completed 2026-08-29 -- design call: rescoped isAntiThrashed/recordPreemption from victim-only to (victim, requester) pair, preserving ADR-005's flip-flop protection for the same requester while letting a different higher-priority requester still preempt. Coordinator independently verified the pair-scoping preserves original intent before approving. PR #92 merged caa8dbf8. NOT yet live-verified, queued)
- [x] T5.3 Regression test using the exact issue scenario (max 3/pod cap, 4+ candidates, high-priority pod). Owner: TBD  Est: 20m  verifies: [UC-108]  deps: [T5.2]  (completed 2026-08-29 -- exact issue scenario test added, PR #92)
- [x] T5.4 `go vet ./... && staticcheck ./... && go test ./... -race -timeout 120s -count=1`. Owner: TBD  Est: 15m  verifies: [infrastructure]  acc: [exit code 0 on all three]  deps: [T5.3]  (completed 2026-08-29 -- go build/vet/staticcheck/test -race all clean on origin/main with E5 merged)
...
- [x] T2.2-T2.3 issue #73 (1 agent, sequential within)         verifies: [UC-103]  (PR #103, merged, not yet live-verified)
- [x] T2.4-T2.5 issue #77 (1 agent, sequential within)         verifies: [UC-103]  lane: agent  (PR #90, merged, not yet live-verified)
- [x] T2.9 issue #88 investigation and fix                     verifies: [UC-105]  lane: agent  (PR #94, merged, HIGH RISK live-verify per R7, not yet done)
- [x] T2.6 flag-skip-list regression coverage                  verifies: [UC-104]  (already covered by pre-existing test, no new code)
- [x] T3.1-T3.5 issue #74 (1 agent carries the epic)            verifies: [UC-102]  (PR #93, merged, not yet live-verified)
- [x] T4.1-T4.2 issue #71 (1 agent)                             verifies: [UC-105]  (PR #101, merged, no live repro exists)
- [x] T4.3-T4.4 issue #75 (1 agent)                             verifies: [UC-106]  (PR #96, merged, not yet live-verified)
- [x] T4.5-T4.6 issue #78 (1 agent)                             verifies: [UC-107]  (PR #97, merged, not yet live-verified)
- [x] T4.7-T4.8 issue #80 manifest endpoint (1 agent)           verifies: [UC-110]  (PR #100, merged, not yet live-verified)
- [x] T4.9 issue #80 signedness bug (1 agent)                   verifies: [UC-108, UC-109]  (PR #95, merged, not yet live-verified)
- [x] T5.1-T5.4 issue #79 (1 agent, lane: agent for T5.2)       verifies: [UC-108]  (PR #92, merged, not yet live-verified)

Wave 2: Epic close-outs, once their Wave 1 tasks converge
- [~] T2.7-T2.8 E2 close-out                                    deps: [T2.3, T2.5, T2.6, T2.9]  (T2.7 done -- suite green; T2.8's merge/release/deploy done, live-verify+close still open)
- [~] T4.10-T4.11 E4 close-out                                  deps: [T4.2, T4.4, T4.6, T4.8, T4.9]  (T4.10 done -- suite green; T4.11's merge/release/deploy done, live-verify+close still open)
- [~] T5.5 E5 close-out                                         deps: [T5.4]  (merge/release/deploy done, live-verify+close still open)
